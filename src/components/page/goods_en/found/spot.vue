				...
				<div class="flex al-ce">
				  ...
				  <div v-show='form.type.val==2'>
					    ...
					    <div class="flex al-ce marginTB20">
							...
							<el-input placeholder="请输入数字" type='number' v-model="form.space" maxlength='20' @keyup.native="key_up('space')" @blur='blur_key_up("space")'> </el-input>
						</div>
						<div class="flex al-ce marginTB20">
							<p class='width150 must'>倍数：</p>
							<el-input placeholder="请输入数字" type='number' v-model="form.multiple" maxlength='20' @keyup.native="key_up('multiple')" @blur='blur_key_up("multiple")'></el-input>
...
<script>
	export default {
		...
		methods: {
			...
			key_up(val){
            	var p2 = parseFloat(this.form[val]).toFixed(2);
            	this.form[val] = p2>=0?(/\.0?$/.test(this.form[val])?this.form[val]:p2.replace(/0$/,'').replace(/\.0$/,'')):''
			},
			blur_key_up(val){
				this.form[val] = this.form[val].replace(/\.0*$/,'')
			}
		},
		watch: {
			goods_details(val){
