<template>
  	<div>
  		...
  		<div class="operation marginB20 flex jc-sb">
  			...
  			<div>
  				<el-button plain class='width120px' type="primary" @click='links("/addGoods_en")'>创建</el-button>
  			</div>
  		</div>
  		<el-row class='goodsList marginB20'>
  			<el-table ref="multipleTable" tooltip-effect="dark"  :data="goodsList" stripe style="width: 100%">
			    ...
			    <el-table-column label="操作">
			      <template slot-scope="scope">
			      	<el-row>
			      		<el-button type="text" size="small" @click='links("/editGoods_en",scope.row.goodsId)' :disabled='edit_power'>编辑</el-button>
			        	<el-button type="text" size="small" @click='delGoods(scope.$index, scope.row)'>删除</el-button>
			      	</el-row>
			      	<el-row>
			      		<el-button type="text" size="small" @click='links("/goodsDetails_en",scope.row.goodsId)'>查看</el-button>
				...
				this.$alert('您创建的商品已在开发者中心关联，必须有关联的对应语言商品！', '创建关联商品', {
		          ...
		          callback: action => {
		          	if(self.$route.path.indexOf('en')>-1){
		          		self.$router.push({path:'/addGoods_en',query:{relation_suk:self.$route.query.relation_suk}})
		          	}else{
		          		self.$router.push({path:'/addGoods',query:{relation_suk:self.$route.query.relation_suk}})
		          	} 
		          }
		        });
			}
